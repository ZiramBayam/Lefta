...
import { getSenderHistory } from "@/lib/contracts";
...
export default function SendHistoryPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const mockAddress = "GDXK...4F2A";
        const data = await getSenderHistory(mockAddress);
        setTransfers(data);
      } catch (err) {
        console.error("Failed to load sender history:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background pb-[100px]">
      <TopAppBar title="Riwayat Kirim" showBackButton />
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-mono-address text-on-surface-variant text-sm">
                      Tx: {transfer.id.slice(0, 12)}...
                    </span>
                  </div>
                ...
                <div className="border-t border-surface-variant pt-3 flex flex-col gap-2">
                  {transfer.splits.map((split, index) => (
                    <div
                      key={index}
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-mono-address text-on-surface-variant text-xs">
                          {truncateAddress(split.recipient, 4)}
                        </span>
                      </div>
